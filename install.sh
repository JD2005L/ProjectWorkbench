...
# Soft memory ceiling for the tmux-server cgroup, which holds every project's
# panes and their long-running agents. Sized from THIS machine's real MemTotal
# rather than written into the unit, because a hardcoded value would be wrong on a
# smaller box and a percentage is wrong under LXC — systemd resolves `%` against
# the PHYSICAL host's RAM, not the container's limit (a 16G CT on a 540G host
# would get a 352G "limit", i.e. none at all). MemoryHigh throttles and reclaims
# under pressure rather than hard-killing, so a busy box degrades instead of
# losing sessions. Seeded only when absent, so operator tuning survives an update.
...
if [ ! -f /etc/systemd/system/pw-tmux-server.service.d/memory.conf ]; then
  pw_mem_high=$(awk '/^MemTotal:/ { printf "%d", $2 * 1024 * 0.75 }' /proc/meminfo 2>/dev/null || true)
...
# Owns the shared tmux server in its own cgroup so no per-project ttyd unit can
# take every project's sessions down with it. Enable BEFORE pw-tmux-persist, which
# orders itself After= this unit and replays the manifest into the server it owns.
# On an existing install with a server already running this start is a safe no-op
# (the keepalive adopts it, the restore is idempotent); the server only actually
# moves into this unit's cgroup at the next reboot or a deliberate restart.
systemctl enable --now pw-tmux-server.service >/dev/null 2>&1 || warn "could not enable pw-tmux-server.service"
