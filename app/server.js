...
app.post(BASE + '/api/tokens', requireAdmin, async (req,res) => {
 try {
  ...
  // The acting account is authority, so it is checked here rather than trusted
  // from the form: a token naming a user who does not exist would fail on first
  // use with nothing to point at, and one naming a disabled account would be a
  // credential for somebody who has been offboarded.
  const actsAs = String(req.body?.actsAs || '').trim();
  if(needsActingUser(scopes)){
   if(!actsAs) return res.status(400).json({ ok:false, error:'A token with session scopes must name the user it acts as' });
   const acting = (await loadUsers()).find(u => u.username === actsAs);
   if(!acting) return res.status(400).json({ ok:false, error:`No such user: ${actsAs}` });
   if(acting.disabled) return res.status(400).json({ ok:false, error:`${actsAs} is disabled; a token cannot act as a disabled account` });
  }
  const projects = req.body?.projects === '*' || req.body?.projects === undefined ? '*'
   : (Array.isArray(req.body.projects) ? req.body.projects.map(String).map(x => x.trim()).filter(Boolean) : []);
  if(projects !== '*'){
   const known = new Set((await loadProjects()).map(p => p.name));
   const missing = projects.filter(name => !known.has(name));
   // A typo'd project name is a token that silently never matches anything.
   if(missing.length) return res.status(400).json({ ok:false, error:'Unknown project(s): '+missing.join(', ') });
  }

  const { plaintext, record } = mintToken({ label, scopes, createdBy: req.user?.username || '', actsAs, projects });
