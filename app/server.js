...
app.get('/api/outbox/:project', requireAuth, requireProjectAccess, async (req,res)=>{ try {
 const p = await projectByName(req.params.project); if(!p) return res.status(404).json({ok:false,error:'Unknown project'});
 ...
 const files = await Promise.all(entries.filter(e=>e.isFile()).map(async e => {
  ...
  return { name: e.name, size: st.size, mtime: st.mtime.toISOString(), path: full, url: `/api/outbox/${encodeURIComponent(p.name)}/file/${encodeURIComponent(e.name)}` };
 }));
 files.sort((a,b)=>b.mtime.localeCompare(a.mtime));
 res.json({ok:true,files});
} catch(e){ res.status(500).json({ok:false,error:e.message||String(e)}); }});
// Download one outbox file (served as an attachment under /api/ so nginx always routes it).
app.get('/api/outbox/:project/file/:name', requireAuth, requireProjectAccess, async (req,res)=>{
 const p = await projectByName(req.params.project); if(!p) return res.status(404).send('Unknown project');
...
app.delete('/api/outbox/:project/file/:name', requireTerminalAccess, async (req,res)=>{ try {
 const p = await projectByName(req.params.project); if(!p) return res.status(404).json({ok:false,error:'Unknown project'});
...
app.delete('/api/outbox/:project', requireTerminalAccess, async (req,res)=>{ try {
 const p = await projectByName(req.params.project); if(!p) return res.status(404).json({ok:false,error:'Unknown project'});
 const outbox = path.join(p.path, '_outbox');
