...
import { SCOPES, mintToken, loadTokens, saveTokens, safeTokenShape, resolveToken,
         presentedToken, tokenHasScope, isKnownScope, looksLikeApiToken,
         needsActingUser, tokenAuthority } from './api-tokens.js';
...
// Per-LAUNCHER credentials. Per-user credentials alone key a project's terminal to
// its primaryUser, so everyone granted access to that project spends the OWNER's
// Claude login and Copilot seat — which is the thing teams actually notice, because
// one person's seat limit gates the whole team. With this on, a tab opened from the
// cockpit runs on the credentials of the PERSON WHO OPENED IT, and the project owner
// remains the identity for everything with no person behind it (the base session,
// the boot reattach, scheduled tasks, bots).
//
// On by default wherever per-user credentials are already on: a workbench that has
// gone to the trouble of giving people their own logins does not want a teammate's
// work billed to the owner. PW_PER_LAUNCHER_CLAUDE=false pins the old owner-keyed
// behaviour without having to revert the app.
//
// Deliberately NOT in app/env-schema.js: that schema is the contract the SHELL
// entrypoints share (project-terminal-start, pw-tmux-restore, the owner restore),
// and none of them can have a launcher — their panes are the project's, not a
// person's. Declaring it there would claim a propagation requirement that does not
// exist.
...
// userTabColors maps a username to a palette name in app/user-colors.js, so a team can
// agree what colour means whom in the cockpit tab strip ("orange is James"). Anyone
// unmapped still gets a stable hashed colour, so this is an override, not a requirement.
const defaultWorkbenchSettings = { permissionMode:'prompt', mcpMode:'isolated', enabledClis:['claude'], updateClis:['claude'], timezone:'', defaultProject:'', userTabColors:{}, deployment:DEFAULT_DEPLOYMENT_SETTINGS };
const PERMISSION_MODES = ['prompt','skip'];
function normalizePermissionMode(v){ return PERMISSION_MODES.includes(v) ? v : 'prompt'; }
const SUPPORTED_CLIS = {
 // authCmd signs in the SHARED box identity from the Setup Wizard's one setup
 // terminal. userAuthCmd signs in ONE PERSON, inside a tab that already carries their
 // own CLAUDE_CONFIG_DIR / COPILOT_HOME, and is deliberately different for Copilot:
 // `gh auth login` would write the shared ~/.config/gh, whereas `copilot login`
 // writes the per-user COPILOT_HOME, which is the whole point.
 claude:  { label:'Claude Code',        pkg:'@anthropic-ai/claude-code', bin:'claude',  authCmd:'claude /login',                              userAuthCmd:'claude /login',   notes:'Anthropic. Wrapper enforces permissions/MCP/shared-memory policy.' },
 codex:   { label:'OpenAI Codex CLI',   pkg:'@openai/codex',             bin:'codex',   authCmd:'codex login',                                userAuthCmd:'',                notes:'OpenAI. Sign in with ChatGPT or set OPENAI_API_KEY. Per-user sign-in is not wired: Codex has no per-user config dir yet.' },
 copilot: { label:'GitHub Copilot CLI', pkg:'@github/copilot',           bin:'copilot', authCmd:'gh auth login --git-protocol=https --web',   userAuthCmd:'copilot login',   notes:'GitHub. Per-user: your own stored token authenticates it, or sign in from your own tab.' }
};
...
app.use((req, res, next) => {
 ...
 // A bearer service token is not ambient credential material - a browser never attaches one
 // on a cross-origin request the way it replays nginx Basic Auth - so the Origin/Referer gate
 // is inapplicable. Authorisation still happens in requireScope; a bogus token gets a 401 there.
 // Shape-checked, not validated: only a value carrying OUR prefix counts as a machine call, so an
 // arbitrary `Authorization: Bearer <anything>` cannot opt a request out of the Origin check.
 if(looksLikeApiToken(presentedToken(req))) return next();
...
async function userCliSignIn(username){
 try {
  return await checkUserCliSignIn({
   fsp: fs, base: USER_CRED_BASE, username,
   owner: await terminalOwner(), currentUid: process.getuid?.() ?? null, runJob: runCredentialJob,
  });
 } catch(e){
  console.warn(`[per-user-claude] ${username}: could not determine CLI sign-in status: ${e?.message || e}`);
  return { claude:false, copilot:false };
 }
}
// How Copilot will authenticate for ONE user, as a state name — never the token.
//
// The token has to be decrypted to classify its TYPE, because "a classic ghp_ PAT"
// and "a fine-grained PAT" are the difference between working and silently failing
// (see app/cli-auth-status.js). Only the resulting state name leaves this function,
// and a token that will not decrypt is reported as its own state rather than being
// mistaken for "no token" — the two need different remedies.
// `hasLogin` is passed IN rather than fetched here so the caller can resolve both
// CLIs from the one status job it already made — this runs per user per request, and
// each job is a privilege-dropped process spawn.
function userCopilotAuth(user, hasLogin){
 let tokenKind, tokenUnreadable = false;
 try { tokenKind = classifyGithubToken(user?.ghToken ? decrypt(user.ghToken) : ''); }
 catch { tokenUnreadable = true; }
 return resolveCopilotAuthState({ tokenKind, hasLogin, tokenUnreadable });
}
...
// Whose credentials a terminal launched BY A PERSON should run on. Returns null for
// "nobody to attribute to" (feature off, no username, or a username with no user
// record — an unauthenticated instance's implicit admin), and the caller then falls
// back to the project owner exactly as before. See app/project-owner.js.
async function launcherCredentialOwner(username){
 if(!PER_LAUNCHER_CLAUDE || !username) return null;
 const users = await loadUsers();
 return resolveLauncherCredentialOwner({ perLauncherEnabled: PER_LAUNCHER_CLAUDE, username, users, decrypt });
}
...
// `launcher` is the signed-in username that ASKED for this pane, when a person did.
// It takes precedence over the project owner (per-launcher mode) so the seat spent is
// the seat of whoever is about to type. Omit it for anything with no person behind it
// — the base session, the boot reattach, scheduled tasks, bots — and the project owner
// is used, which is the pre-per-launcher behaviour.
async function credentialContext(project, launcher = ''){
 const off = { tokens: [], shellArgs: DEFAULT_SHELL_ARGS, key: CREDENTIALS_OFF, username: '' };
 let owner;
 try {
  owner = await launcherCredentialOwner(launcher);
  // No launcher identity: fall back to the project's owner. This is NOT the shared
  // login (that is what AC1 forbids) — it is the same per-user identity this project
  // used before per-launcher mode existed.
  if(!owner) owner = await projectCredentialOwner(project);
 } catch(e){
  throw new Error(`[per-user-claude] project "${project?.name || '?'}" cannot resolve its credential owner (launcher "${launcher || ''}", primaryUser "${project?.primaryUser || ''}"): ${e?.message || e}. Refusing to fall back to the shared Claude/GitHub login.`);
 }
 if(!owner) return off;
 try {
  ...
  throw new Error(`[per-user-claude] project "${project.name}" (identity "${owner.username}") credential materialization failed: ${e?.message || e}. Refusing to fall back to the shared Claude/GitHub login.`);
 }
}
...
const apiTokensPath = process.env.PW_API_TOKENS_PATH || '/etc/project-workbench/api-tokens.json';
async function withApiTokensLock(mutate){
 // Takes the lifecycle lock for the same reason the per-user landing memory does: a
 // read-modify-write must not clobber OTHER tokens' entries (notably lastUsedAt).
 return credentialDomain.withLocks(['lifecycle'], async () => {
  const tokens = await loadTokens(apiTokensPath);
  const result = await mutate(tokens);
...
// never inherit the dashboard's admin authority, only the scopes it explicitly carries.
function requireScope(scope){
 return async (req, res, next) => {
  ...
  if(!token){
   await audit('api_token_denied', { reason:'unknown-or-revoked', scope }, req);
   return res.status(401).json({ ok:false, error:'Invalid or revoked token' });
  }
  if(!tokenHasScope(token, scope)){
   await audit('api_token_denied', { reason:'missing-scope', tokenId:token.id, label:token.label, scope }, req);
   return res.status(403).json({ ok:false, error:`Token lacks required scope "${scope}"` });
  }
  // Best-effort usage stamp. A lost write costs a timestamp, never authorisation.
  ...
  req.apiToken = token;
  await audit('api_token_used', { tokenId:token.id, label:token.label, scope }, req);
  return next();
 };
}
...
// The ROLE half of terminal access, without a project. Used by routes where there is
// no project in the URL to authorise because the server picks one from what this user
// can already see (see POST /api/me/cli-login): with no caller-supplied project name,
// filterProjectsForUser IS the authorisation, and there is nothing for
// requireProjectAccess to check.
function requireTerminalRole(req, res, next){
 if(!req.user) return requireAuth(req, res, next);
...
function requireTerminalAccess(req, res, next){
 return requireTerminalRole(req, res, () => requireProjectAccess(req, res, next));
}
...
// Category tags are presentation-only grouping for the rail's filter. Comma-separated in the form,
// stored as an array on the project. '|' is stripped because the rail serializes the list into a
// pipe-joined data attribute; dedup is case-insensitive with the first-seen casing kept.
function parseCategories(raw){
 const out = [];
 const seen = new Set();
 for(const part of String(raw ?? '').split(',')){
  const name = part.replace(/[|\u0000-\u001f\u007f]/g,'').replace(/\s+/g,' ').trim().slice(0,40);
  if(!name) continue;
...
// A tmux-adjacent helper run as the account that OWNS the sessions, by the same rule as tmux()
// above: the dashboard is root, and a tool that talks to the tmux server must reach it as the
// terminal account or it addresses a different server entirely. Never used for a shell string —
// argv only, so nothing here is parsed by a shell.
function tmuxOwned(file, args, opts={}){
 if(DEPLOY_MODE === 'container') return execFileAsync(file, args, {timeout:10000,...opts});
 return execFileAsync('sudo',['-u',hostTerminalUser(process.env),file,...args],{timeout:10000,...opts});
}
function parseTmuxWindows(stdout){
 return String(stdout || '').split('\n').filter(Boolean).map(line=>{
  ...
  // historical positions; these cannot be knocked out of place by a name.
  const tail = parts.slice(-7);
  const [windowId, hibSid, hibWindow, waiting, paneCommand, panePid, credUser] = parts.length >= 13 ? tail : ['', '', '', '', '', '', ''];
  ...
  return {
   ...
   autoResume: hibernationMarkers && /^\d+(:\d+)?$/.test(waiting || ''),
  };
...
async function listTmuxWindows(project){
 const { stdout } = await tmux(['list-windows','-t',tmuxSession(project),'-F','#{window_index}|#{window_name}|#{window_active}|#{window_bell_flag}|#{session_attached}|#{window_activity}|#{window_id}|#{@pw_claude_sid}|#{@pw_claude_hib_win}|#{@pw_claude_waiting}|#{pane_current_command}|#{pane_pid}|#{@pw_cred_user}']);
 const now = Math.floor(Date.now()/1000);
 const session = tmuxSession(project);
 const windows = parseTmuxWindows(stdout).map(({ placeholderInPane, ...w }) => ({
  ...w,
  hibernated: placeholderInPane && placeholderState(w.panePid) === 'waiting',
  working: computeWorking(session, w, now),
 }));
 return withCredColors(windows);
}
// Resolve each window's owner colour at READ time, never at stamp time, so changing
// the team's colour mapping repaints the strip on the next poll instead of requiring
// every tab to be recreated.
async function withCredColors(windows){
 if(!windows.some(w => w.credUser)) return windows;
 const map = await userTabColorMap();
 return windows.map(w => (w.credUser
  ? { ...w, credColor: userTabColorCss(resolveUserTabColor(w.credUser, map)) }
  : w));
}
...
// A tab's identity is a property of the WINDOW, not of the session: with
// per-launcher credentials two people's tabs legitimately coexist in one project's
// session, each created with its own CLAUDE_CONFIG_DIR / COPILOT_HOME / GH_TOKEN.
// So each window records, at creation, whose credentials its pane was given —
// @pw_cred_user for the person (what the cockpit colours the tab by) and
// @pw_cred_key for the fingerprint those credentials hashed to.
//
// Recording it is what makes mixed attribution SAFE rather than merely allowed: the
// alternative to a per-window record is a strip of tabs that all look alike while
// spending different people's seats. A window whose identity cannot be recorded is
// therefore destroyed rather than left running unlabelled (see newTmuxWindow).
const CRED_USER_OPTION = '@pw_cred_user';
// Same deliberate omission of `-q` as readSessionCredKey, for the same reason: a
// genuinely-unset option must be distinguishable from a vanished window or a
// control-plane error. Its only caller reads back a value it has just written, where
// "unset" is not a legitimate answer at all — it comes back as '' and fails the
// comparison, which is the correct outcome. Keeping the distinction anyway is what
// stops a control-plane error from being silently read as a successful stamp.
async function readWindowCredOption(target, option){
 try {
  const { stdout } = await tmux(['show-options','-w','-t',target,'-v',option]);
  return { ok:true, value: String(stdout || '').trim() };
 } catch(e){
  const stderr = String(e?.stderr || e?.message || '');
  if(/invalid option/i.test(stderr)) return { ok:true, value:'' };
  return { ok:false, error: stderr.trim() || String(e) };
 }
}
// Stamps BOTH options and reads them back, so a set-option that exits zero but is
// silently dropped cannot leave a pane running under an unrecorded identity.
async function stampWindowCredIdentity(target, key, username){
 for(const [option, value] of [[CRED_KEY_OPTION, key || ''], [CRED_USER_OPTION, username || '']]){
  await tmux(['set-option','-w','-t',target,option,value]);
  const verify = await readWindowCredOption(target, option);
  if(!verify.ok || verify.value !== value){
   throw new Error(`could not verify the ${option} stamp on window "${target}" (wrote ${JSON.stringify(value)}, read back ${verify.ok ? JSON.stringify(verify.value) : `unreadable: ${verify.error}`})`);
  }
 }
}
// Best-effort for the paths that create a window as part of a larger operation:
// the stamp is worth having but a session must not fail to come up over a cosmetic
// label. newTmuxWindow, where the whole point IS the per-person identity, uses the
// strict version above.
async function stampWindowCredIdentityQuietly(target, key, username){
 if(!username) return;   // shared login: nothing to attribute, leave the tab uncoloured
 try { await stampWindowCredIdentity(target, key, username); }
 catch(e){ console.warn(`[per-user-claude] could not label window "${target}" with its credential identity: ${e?.message || e}`); }
}
...
async function ensureTmuxSession(p){
 ...
 const { stdout: firstId } = await tmux(['new-session','-d','-s',sess,'-c',cwd,'-n',firstName,'-P','-F','#{window_id}',...env,'bash',...cred.shellArgs]);
 await stampSessionCredKey(sess, cred.key);
 // The session's own windows carry the same identity label as any tab opened later,
 // so the strip is consistent from the first paint: an unlabelled tab means the shared
 // box login, not "unknown".
 await stampWindowCredIdentityQuietly(String(firstId || '').trim() || `${sess}:${firstName}`, cred.key, cred.username);
 if(tabs[0]?.cmd?.trim()) await tmux(['send-keys','-t',`${sess}:${firstName}`,tabs[0].cmd.trim(),'C-m']);
 for(let i=1; i<tabs.length; i++){
  const t = tabs[i]; if(!t?.name) continue;
  const { stdout: tabId } = await tmux(['new-window','-t',sess,'-c',cwd,'-n',t.name,'-P','-F','#{window_id}',...env,'bash',...cred.shellArgs]);
  await stampWindowCredIdentityQuietly(String(tabId || '').trim() || `${sess}:${t.name}`, cred.key, cred.username);
...
// `launcher` is the signed-in user who asked for this tab; '' for anything a person
// did not click (scheduled tasks, bots). Per-launcher mode makes that identity the
// pane's, which is why this function no longer requires the new window to match the
// SESSION's fingerprint.
//
// That requirement used to be the guard against mixed-attribution panes, and it was
// the right guard while a session could only have one identity: a window created
// under different credentials than its siblings, with nothing recording which, is
// genuinely unsafe to hand someone. Per-launcher mode makes the mix the POINT, so the
// guard moves rather than disappears — from "every pane in a session must match" to
// "every pane records the identity it was created with", stamped and verified below,
// and surfaced as the tab's colour. A window whose identity cannot be recorded is
// killed, not handed over unlabelled.
//
// (It also removes a bug the old rule caused in practice: once PW_PER_USER_CLAUDE was
// switched on, every grandfathered session refused to open a new tab at all —
// "existing session credentials are stale … recycle required" — because the live
// session predated the flag.)
async function newTmuxWindow(p,name='new task',cmd='',launcher=''){
 const safeName = String(name || 'new task').replace(/[\r\n\t]/g,' ').trim().slice(0,80) || 'new task';
 const sess = tmuxSession(p.name);
 const cred = await credentialContext(p, launcher);
 ...
 const { stdout: newIdx } = await tmux(['new-window','-t',sess,'-c',p.path,'-n',safeName,'-P','-F','#{window_index}|#{window_id}',...winEnv,'bash',...cred.shellArgs]);
 const [idx, newWindowId] = String(newIdx || '').trim().split('|');
 // Address the stamp by window ID, never by index: indexes shift as windows are
 // closed, and a stamp that lands on a neighbouring window would mislabel two tabs
 // at once. Recorded BEFORE any command is sent, so a pane is never running an agent
 // on someone's account before that account is written down.
 if(cred.username){
  try {
   await stampWindowCredIdentity(newWindowId || `${sess}:${idx}`, cred.key, cred.username);
  } catch(e){
   await tmux(['kill-window','-t',newWindowId || `${sess}:${idx}`]).catch(()=>{});
   throw new Error(`[per-user-claude] project "${p.name}": created a window for "${cred.username}" but could not record whose credentials it runs on (${e?.message || e}). The window was closed rather than left unlabelled — try again.`);
  }
 }
...
async function projectSignals(p){
 ...
 try {
  ...
  return {
   ...
   // Reported so the rail can say a project is holding hibernated conversations instead of
   // rendering it identically to a live idle one. A hibernated window is silent by construction —
   // no bell, no output — so without this the UI is confidently wrong rather than merely quiet.
   hibernated: ws.filter(w => w.hibernated).length,
  };
 } catch { return { bell:false, working:false, attached:false, hibernated:0 }; }
}
...
.tabStrip .tab.hibernated{opacity:.62;border-style:dashed}
.tabStrip .tab.hibernated:hover{opacity:1}
.tabStrip .tab .zzz{font-size:10px;line-height:1;opacity:.85;flex:0 0 auto}
.tabStrip .tab.resuming{opacity:1;border-style:solid;border-color:var(--cyan);color:#cdefff}
.tabStrip .tab.resuming .zzz{animation:pwLivePulse 1.2s ease-in-out infinite}
...
/* Whose account a tab spends. Drawn as its own swatch plus a left edge, deliberately
   NOT as the tab's border colour: --cyan (resuming), --amber2 (attention) and --ok
   (working) already mean things there, and an identity must not be able to hide a
   state or be mistaken for one. Rules come after those states so the left edge keeps
   the person's colour while the rest of the border still shows the state. */
.tabStrip .tab .who{width:7px;height:7px;flex:0 0 7px;border-radius:2px;background:var(--cu,var(--line2))}
.tabStrip .tab.cu{border-left-width:3px;border-left-style:solid;border-left-color:var(--cu);padding-left:6px}
.tabStrip .tab.cu.active,.tabStrip .tab.cu.attention,.tabStrip .tab.cu.resuming{border-left-color:var(--cu)}
...
.railFilterBtn.filtering .railFilterIco{filter:drop-shadow(0 0 6px rgba(56,189,248,.6))}
.railFilterIco{display:inline-grid;place-items:center;width:26px;height:26px;flex:0 0 26px;font-size:13px}
.railFilterLabel{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;opacity:0;transform:translateX(-8px);transition:opacity .22s,transform .22s}
.railFilterBtn .chev{color:var(--faint);font-size:14px;opacity:0;transition:opacity .2s,transform .3s cubic-bezier(.32,.72,.24,1)}
.railFilterBtn[aria-expanded="true"] .chev{transform:rotate(180deg)}
.railFilterMenu{position:absolute;left:8px;right:8px;top:calc(100% + 2px);z-index:20;background:var(--elev2);border:1px solid var(--line2);border-radius:12px;box-shadow:0 22px 44px -12px rgba(0,0,0,.9);padding:6px;display:flex;flex-direction:column;gap:2px;max-height:50vh;overflow-y:auto;scrollbar-width:thin;scrollbar-color:var(--line) transparent}
...
function railHtml(projects, currentName, user, deployConfigured=false){
 ...
 const projCats = p => Array.isArray(p.categories) ? p.categories : [];
...
if(sub)sub.textContent=(p.name===CUR)?(p.working?'working…':'active session'):(lit?'finished — click to view':(p.working?'working…':(p.hibernated?'hibernated — open to resume':'')));
...
const addProjectHandler = async (req,res,next)=>{ try {
...
app.get(BASE + '/api/projects/status', requireAuth, async (req,res)=>{ try {
 ...
 const out = await Promise.all(projects.map(async p => {
  ...
  return { name: p.name, ...pend, bell: sig.bell, working: sig.working, hibernated: sig.hibernated || 0, pending: pend.pending || sig.bell, credentialsStale: await credentialsStale(p) };
 }));
...
app.post(BASE + '/api/term/:project/windows', requireTerminalAccess, async (req,res)=>{ try {
 const p = await requireProject(req,res); if(!p) return;
 // req.user is the person who clicked "+", and in per-launcher mode that is whose
 // Claude login and Copilot seat this tab will spend. The IMPLICIT_ADMIN of an
 // instance with auth disabled is nobody in particular — attributing its tabs to a
 // user record that happens to share its name would be a fabricated identity, so it
 // is passed as no launcher and keeps the project owner.
 await newTmuxWindow(p, req.body?.name || 'new task', req.body?.cmd || '', req.user?.implicit ? '' : (req.user?.username || ''));
...
// Resume a hibernated conversation in place, at the request of someone looking at it.
//
// Hibernation frees memory by stopping an idle Claude and leaving a placeholder in its window; the
// tmux wake hooks resume it when a client attaches or the window is selected. But the placeholder
// stops advertising itself once its automatic retries are spent, and from then on no hook can reach
// it: the window is live, silent, indistinguishable from an idle one, and only a person pressing
// Enter in the pane gets the conversation back. That is the state the cockpit could neither see nor
// leave — it just looked healthy. This is the person, pressing Enter, through the same script and
// the same safety gates the hooks use.
//
// requireTerminalAccess, exactly like every other window route: this sends a keystroke into a
// terminal, so it is the terminal's own authorization, not a weaker one.
app.post(BASE + '/api/term/:project/windows/:index/wake', requireTerminalAccess, async (req,res)=>{ try {
 const p = await requireProject(req,res); if(!p) return;
 const index = Number(req.params.index);
 const windows = await listTmuxWindows(p.name);
 const w = windows.find(x => x.index === index);
 if(!w) return res.status(404).json({ok:false,error:'Unknown window'});
 // Not an error, and deliberately not a refusal the UI has to special-case: a window that woke
 // between the poll and the click has simply arrived where the caller wanted it.
 if(!w.hibernationMarkers) return res.json({ok:true,woke:false,reason:'this window is not hibernated',windows});
 if(!w.windowId) return res.status(409).json({ok:false,error:'this window could not be identified'});
 const { stdout: sockOut } = await tmux(['display-message','-p','#{socket_path}']).catch(()=>({stdout:''}));
 const socketPath = String(sockOut || '').trim();
 let woke = false, reason = '';
 try {
...
app.get(BASE + '/api/setup/state', requireAdmin, async (_req,res)=>{ try {
 ...
 // from (on). Without it the sign-in controls describe the wrong thing.
 res.json({ ok:true, settings:publicWorkbenchSettings(settings), clis, updateStamp, perUserClaude: PER_USER_CLAUDE });
} catch(e){ res.status(500).json({ok:false,error:e.message||String(e)}); }});
...
app.post(BASE + '/api/setup/state', requireAdmin, async (req,res)=>{ try {
 ...
 // Normalized before it is stored, so an unknown colour name is rejected at the
 // boundary rather than persisted and silently ignored on every later read.
 if(body.userTabColors && typeof body.userTabColors === 'object') patch.userTabColors = normalizeUserTabColors(body.userTabColors);
...
// Label the windows of sessions that PREDATE per-window identity stamps.
//
// A session created before the stamp existed runs on perfectly good credentials — it
// just never recorded whose, so the cockpit shows its tabs uncoloured. Recycling would
// fix that by recreating the session, but recycling kills whatever is running in it,
// which is far too high a price for a label.
//
// It does not need to be paid. A stamp is a tmux window OPTION: writing one is metadata,
// invisible to the pane's process. So the label can be backfilled in place on a live
// session, provided the label is TRUE — and that is the whole design of this function:
//
//   * the session's own stamped fingerprint must VERIFIABLY equal the fingerprint its
//     current owner resolves to today. That is what proves the panes really are running
//     on that person's credentials, rather than us assuming it;
//   * anything that cannot be established — an unreadable stamp, an owner who no longer
//     resolves, a stale stamp (the panes are on OLD credentials, so the current owner's
//     name would be a lie) — is SKIPPED with the reason, never guessed at. A stale
//     session still needs a deliberate recycle, which is exactly what it needed before;
//   * a window that ALREADY carries a label is left alone. With per-launcher credentials
//     a session legitimately holds several identities, and overwriting one of those with
//     the project owner's name would replace a true label with a false one.
//
...
app.post(BASE + '/api/setup/heal/session-labels', requireAdmin, async (req,res)=>{ try {
 if(!PER_USER_CLAUDE) return res.status(409).json({ ok:false, error:'Per-user credentials are off, so terminals have no per-person identity to label.' });
 const results = await backfillSessionIdentityLabels();
 const total = results.reduce((n,r) => n + (r.labelled || 0), 0);
 const needRecycle = results.filter(r => r.status === 'stale').map(r => r.project);
 const unresolved = results.filter(r => ['unverifiable','owner-unresolved','partial'].includes(r.status));
 const lines = [
  `Labelled ${total} existing tab${total === 1 ? '' : 's'} across ${results.filter(r => r.status === 'labelled').length} live session${results.filter(r => r.status === 'labelled').length === 1 ? '' : 's'} — no process was touched.`,
  ...results.filter(r => r.status === 'labelled' && (r.labelled || r.kept)).map(r => `  ${r.project}: ${r.labelled} labelled as ${r.owner}${r.kept ? `, ${r.kept} already labelled and left alone` : ''}`),
 ];
 if(needRecycle.length) lines.push(`Skipped (running on older credentials than their owner resolves to today, so the current owner's name would be wrong — recycle to migrate): ${needRecycle.join(', ')}`);
 for(const r of unresolved) lines.push(`Skipped ${r.project} (${r.status}): ${r.detail || 'could not establish whose credentials these panes use'}`);
 await audit('session_labels_backfilled', { labelled: total, results }, req);
 res.json({ ok:true, message: lines.join('\n'), results });
} catch(e){ res.status(500).json({ ok:false, error: e.message || String(e) }); }});
...
.meBtn.link{background:none;color:var(--cyan);padding:.42rem .2rem;text-decoration:underline dotted;font-size:.8rem}
.meBtn.link:hover{text-decoration-style:solid}
...
function render(s){
 ...
 for(const c of s.clis){
  ...
  if(c.auth.canSelfSignIn&&s.canOpenTerminal){
   /* Owed vs merely possible: signing in for the first time is the task, so it gets a
      button; re-authenticating something that already works is a quiet link. */
   const b=document.createElement('button');b.type='button';
   b.className=c.auth.needsSignIn?'meBtn':'meBtn link';
   b.textContent=c.auth.needsSignIn?'Sign in':'Sign in again';
...
//
// The Setup Wizard's sign-in (below) authenticates the ONE shared box identity: it
// sends the login command into a single shared setup terminal that carries no per-user
// credential environment. Since per-launcher credentials landed, that identity is no
// longer what runs anybody's project tabs — it is the seed a new per-user directory is
// built from — so it cannot be how a person signs in.
//
// This route needs no new terminal infrastructure, because the per-launcher path is
// already exactly the mechanism required: a tab created by a person runs with THEIR
// config dirs, so a login command sent into a tab they opened lands in their own
// directory. It is deliberately SELF-SERVICE — it acts only for req.user and takes no
// username — because an admin pressing it for someone else would create the tab on the
// ADMIN's credentials and sign the wrong person in.
app.post(BASE + '/api/me/cli-login', requireTerminalRole, async (req,res)=>{ try {
 const cli = String(req.body?.cli || '');
 const cfg = SUPPORTED_CLIS[cli];
 if(!cfg) return res.status(400).json({ok:false,error:'Unknown CLI'});
 if(!cfg.userAuthCmd) return res.status(400).json({ok:false,error:`${cfg.label} has no per-user sign-in yet: it has no per-user config directory, so a login would be written to the shared one.`});
 if(!PER_USER_CLAUDE) return res.status(409).json({ok:false,error:'Per-user credentials are off (PW_PER_USER_CLAUDE), so every terminal shares one login. Sign in from Settings → Setup Wizard instead.'});
 // The implicit admin of an auth-disabled instance is a placeholder, not a person:
 // there is no identity to sign in, and guessing one would create a tab on a user
 // record that merely shares its name.
 if(req.user.implicit) return res.status(409).json({ok:false,error:'Sign in to the dashboard first — an anonymous session has no identity to authenticate.'});

 if(cli === 'copilot'){
  // Copilot reads GH_TOKEN ahead of any stored login, so if this person has a stored
  // token a login here cannot take effect. Refusing with the reason is the useful
  // answer; opening a terminal that silently changes nothing is not.
  const me = (await loadUsers()).find(u => u.username === req.user.username);
  const verdict = copilotLoginWouldTakeEffect(userCopilotAuth(me || {}, false).state);
  if(!verdict.ok) return res.status(409).json({ok:false,error:verdict.reason});
 }

 const projects = filterProjectsForUser(await loadProjects(), req.user);
 if(!projects.length) return res.status(409).json({ok:false,error:'You have no projects yet — a sign-in runs inside a project terminal. Ask an admin for access to one.'});
 const preferred = await lastProjectForUser(req.user.username);
 const target = projects.find(p => p.name === preferred) || projects[0];

 // Same path as the "+" menu, with this person as the launcher, so the pane carries
 // their CLAUDE_CONFIG_DIR / COPILOT_HOME and the login persists into their own dir.
 const index = await newTmuxWindow(target, `${cfg.label} login`, cfg.userAuthCmd, req.user.username);
 await audit('cli_user_login_started', { cli, project: target.name }, req);
...
function cliCellHtml(u,cli){
 ...
 /* Every state renders as a pill, including the neutral ones: a column that mixes pills
    with bare text reads as though only some rows have a real answer. */
 const pill='<span class="role-pill"'+(tone?' '+tone:'')+' title="'+esc(cell.detail)+'">'+esc(cell.label)+'</span>';
 /* The action appears only when one is OWED (needsSignIn), and only on your own row.
    NOT when already signed in: a "Sign in" button beside a cell reading "signed in"
    makes a reader doubt the status. Re-authenticating is still possible, from your own
    page, where it is not sitting in a list of everybody else's states. And never on
    someone else's row — the sign-in runs in a terminal carrying the presser's
    credentials, so it would sign in the presser. Where the fix is an admin one (a token
    Copilot refuses) the Git-token column is that means. */
 const mine=u.username===PW_ME;
 const act=(mine&&cell.needsSignIn)
  ? '<button class="cellAct" data-signin="'+esc(cli.key)+'" title="Open a tab in one of your projects running this login, so the credential lands in YOUR own config dir">sign in</button>'
  : '';
...
uTable.addEventListener('click',async e=>{const t=e.target;if(t.dataset.ghoauth){
 ...
 t.disabled=true;setStatus(uStatus,'Opening a sign-in tab…');
 try{const r=await fetch('${BASE}/api/me/cli-login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cli:t.dataset.signin})});const j=await r.json();
  if(!j.ok)throw new Error(j.error||('HTTP '+r.status));
  setStatus(uStatus,'Running '+j.command+' in '+j.project+' — complete it in that terminal.');
  window.open(j.url,'_blank','noopener');
...
const enabled=new Set(state.settings.enabledClis||[]);const upd=new Set(state.settings.updateClis||[]);for(const c of Object.values(state.clis)){const row=document.createElement('div');row.className='cli-row';row.dataset.cli=c.key;row.innerHTML='<div class="meta"><span class="label">'+esc(c.label)+'</span><span class="version'+(c.installed?' installed':'')+'">'+esc(c.version)+'</span>'+(c.authenticated&&!state.perUserClaude?'<span class="signed-in" title="Credentials detected on disk">Signed in</span>':'')+'<span class="cli-checked'+(c.lastUpdateOk===false?' bad':'')+'" title="When the auto-updater last checked this CLI">'+(c.lastUpdate==='never'?'never checked':(c.lastUpdateOk===false?'check failed \u00b7 ':'checked ')+esc(c.lastUpdate))+'</span>'+'</div><div class="checks"><label><input type="checkbox" class="en"'+(enabled.has(c.key)?' checked':'')+'>Enable</label><label><input type="checkbox" class="up"'+(upd.has(c.key)?' checked':'')+'>Auto-update</label></div><div class="actions"><button class="button secondary tiny inst">'+(c.installed?'Update':'Install')+'</button>'+(state.perUserClaude?'':'<button class="button tiny auth">'+(c.authenticated?'Reauthenticate':'Sign in')+'</button>')+'</div><div class="note">'+esc(c.notes)+'</div>';row.querySelector('.inst').onclick=async()=>{const btn=row.querySelector('.inst');btn.disabled=true;btn.textContent='Installing…';setStatus(cliStatus,'');try{const r=await fetch('${BASE}/api/setup/cli/install',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cli:c.key})});const j=await r.json();if(!j.ok)throw new Error(j.error||'install failed');setStatus(cliStatus,c.label+': '+j.version);loadState();loadSystem()}catch(e){setStatus(cliStatus,e.message,true);loadState()}};const authBtn=row.querySelector('.auth');if(authBtn)authBtn.onclick=async()=>{const btn=authBtn;btn.disabled=true;setStatus(cliStatus,'');try{const r=await fetch('${BASE}/api/setup/cli/auth',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cli:c.key})});const j=await r.json();if(!j.ok)throw new Error(j.error||'auth start failed');if(authFrame.src.indexOf('${BASE}/pty/_setup/')<0)authFrame.src='${BASE}/pty/_setup/';authFrame.classList.remove('hidden');authHint.textContent='Running: '+j.command+' — complete the prompts in the terminal below.'}catch(e){setStatus(cliStatus,e.message,true)}finally{btn.disabled=false}};cliRows.appendChild(row)}}const PERM_HELP={prompt:'Claude pauses and asks before each tool use (file edit, shell command, etc.). Safest default.',skip:'<b>Warning:</b> passes <code>--dangerously-skip-permissions</code>. Claude runs every tool unattended. Anyone with dashboard access effectively has shell on this box.'};const MCP_HELP={inherit:'Use the MCP servers configured on your Anthropic account.',isolated:'Use an empty MCP config so no external MCP servers load.',custom:'Use a custom MCP JSON via <code>PW_MCP_CONFIG</code>.'};function renderEnv(){permMode.value=state.settings.permissionMode||'prompt';mcpMode.value=state.settings.mcpMode||'isolated';renderEnvHelp()}function renderEnvHelp(){document.getElementById('permHelp').innerHTML=PERM_HELP[permMode.value]||'';document.getElementById('permHelp').classList.toggle('warn',permMode.value==='skip');document.getElementById('mcpHelp').innerHTML=MCP_HELP[mcpMode.value]||''}permMode.addEventListener('change',renderEnvHelp);mcpMode.addEventListener('change',renderEnvHelp);envSave.onclick=async()=>{envSave.disabled=true;setStatus(envStatus,'Saving…');try{const enabledClis=[...cliRows.querySelectorAll('.cli-row')].filter(r=>r.querySelector('.en').checked).map(r=>r.dataset.cli);const updateClis=[...cliRows.querySelectorAll('.cli-row')].filter(r=>r.querySelector('.up').checked).map(r=>r.dataset.cli);const r=await fetch('${BASE}/api/setup/state',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({permissionMode:permMode.value,mcpMode:mcpMode.value,enabledClis,updateClis})});const j=await r.json();setStatus(envStatus,j.ok?'Saved.':'Error: '+j.error,!j.ok)}catch(e){setStatus(envStatus,e.message,true)}finally{envSave.disabled=false}};async function heal(url,btn){btn.disabled=true;healOut.className='heal-out show';healOut.textContent='Working…';try{const r=await fetch(url,{method:'POST'});const j=await r.json();if(!j.ok)throw new Error(j.error||'failed');healOut.textContent=j.message||'OK';healOut.className='heal-out show'}catch(e){healOut.textContent=e.message;healOut.className='heal-out show err'}finally{btn.disabled=false;loadSystem()}}healNginx.onclick=()=>heal('${BASE}/api/setup/heal/nginx',healNginx);healDirs.onclick=()=>heal('${BASE}/api/setup/heal/dirs',healDirs);const healLabels=document.getElementById('healLabelsBtn');if(healLabels)healLabels.onclick=()=>heal('${BASE}/api/setup/heal/session-labels',healLabels);loadState();loadSystem();const tokLabel=document.getElementById('tokLabel');const tokActsAs=document.getElementById('tokActsAs');const tokAllProjects=document.getElementById('tokAllProjects');const tokProjectList=document.getElementById('tokProjectList');const tokScopes=document.getElementById('tokScopes');
let PW_TOKEN_USERS=[],PW_TOKEN_SCOPES=[],PW_TOKEN_LIST=[];
/* The project list belongs to the SELECTED ACCOUNT, because a token can never
   exceed the person it acts as — offering a project they cannot reach would offer
   a tick that means nothing. Choosing nobody (a non-session token) leaves nothing
   to scope, so the picker says so rather than listing every project. */
function projectsFor(username){const u=PW_TOKEN_USERS.find(x=>x.username===username);return u?u.projects||[]:[]}
function renderProjectPicker(){
 const who=tokActsAs.value;
 const mine=projectsFor(who);
 if(!who){tokProjectList.innerHTML='<span class="muted">Choose an account first — project scope only applies to a token that acts as somebody.</span>'}
 else if(!mine.length){tokProjectList.innerHTML='<span class="muted">'+esc(who)+' has no project grants, so a session token for them could reach nothing.</span>'}
 else {tokProjectList.innerHTML=mine.map(p=>'<label><input type="checkbox" class="tokProject" value="'+esc(p)+'">'+esc(p)+'</label>').join('')}
 syncProjectPicker();
}
/* "All projects" and a per-project tick are the same decision expressed two ways,
   so only one of them is live at a time — a greyed list beside a ticked "all" is
   the state people misread as "all of these, plus those". */
function syncProjectPicker(){tokProjectList.classList.toggle('disabled',tokAllProjects.checked||!tokActsAs.value)}
tokAllProjects.addEventListener('change',syncProjectPicker);
tokActsAs.addEventListener('change',renderProjectPicker);const tokCreate=document.getElementById('tokCreate');const tokList=document.getElementById('tokList');const tokStatus=document.getElementById('tokStatus');const tokNew=document.getElementById('tokNew');const tokConnect=document.getElementById('tokConnect');const tokMcp=document.getElementById('tokMcp');const tokCurl=document.getElementById('tokCurl');const tokNewVal=document.getElementById('tokNewVal');const tokCopy=document.getElementById('tokCopy');function fmtWhen(v){if(!v)return 'never';try{return new Date(v).toLocaleString()}catch(e){return v}}async function loadTokens(){try{const r=await fetch('${BASE}/api/tokens',{cache:'no-store'});const j=await r.json();if(!j.ok)throw new Error(j.error||'load failed');if(!tokScopes.dataset.built){tokScopes.innerHTML='<legend>Scopes</legend>'+(j.scopes||[]).map(sc=>'<label><input type="checkbox" class="tokScope" value="'+esc(sc)+'">'+esc(sc)+'</label>').join('');tokScopes.dataset.built='1'}
   PW_TOKEN_USERS=j.users||[];PW_TOKEN_SCOPES=j.scopes||[];PW_TOKEN_LIST=j.tokens||[];
   if(!tokActsAs.dataset.built){tokActsAs.innerHTML='<option value="">— nobody (not a session token) —</option>'+PW_TOKEN_USERS.map(u=>'<option value="'+esc(u.username)+'">'+esc(u.username)+'</option>').join('');tokActsAs.dataset.built='1'}
   renderProjectPicker();
   const ts=j.tokens||[];if(!ts.length){tokList.innerHTML='<span class="empty">No tokens yet.</span>';return}tokList.innerHTML=ts.map(t=>'<div class="tok-row'+(t.disabled?' revoked':'')+'">'+'<div><b>'+esc(t.label||'(unlabelled)')+'</b>'+(t.disabled?' <span class="tok-tag">revoked</span>':'')+'</div>'+'<div class="muted">scopes: '+esc((t.scopes||[]).join(', ')||'none')+'</div>'+(t.actsAs?'<div class="muted">acts as <b>'+esc(t.actsAs)+'</b> | projects: '+esc(t.projects==='*'?'* (whatever that account can reach)':(t.projects||[]).join(', ')||'none')+'</div>':'')+'<div class="muted">created '+esc(fmtWhen(t.createdAt))+(t.createdBy?' by '+esc(t.createdBy):'')+' | last used '+esc(fmtWhen(t.lastUsedAt))+'</div>'+(t.actsAs&&t.updatedAt?'<div class="muted">scopes last changed '+esc(fmtWhen(t.updatedAt))+(t.updatedBy?' by '+esc(t.updatedBy):'')+'</div>':'')+(t.disabled?'':'<div class="tok-actions"><button class="button secondary tiny tokEdit" data-id="'+esc(t.id)+'" type="button">Edit</button><button class="button secondary tiny tokRevoke" data-id="'+esc(t.id)+'" type="button">Revoke</button></div><div class="tok-editor" id="tokEdit-'+esc(t.id)+'" hidden></div>')+'</div>').join('');tokList.querySelectorAll('.tokEdit').forEach(b=>{b.onclick=()=>{
    const t=(PW_TOKEN_LIST||[]).find(x=>x.id===b.dataset.id);if(!t)return;
    const box=document.getElementById('tokEdit-'+t.id);
    if(!box.hidden){box.hidden=true;box.innerHTML='';b.textContent='Edit';return}
    b.textContent='Cancel';box.hidden=false;
    /* Editing changes what a token MAY DO; it never touches the digest, so the
       client already holding it keeps working. That is the point of the feature,
       and it is why the form says so out loud. */
    const own=projectsFor(t.actsAs);
    const all=t.projects==='*';
    box.innerHTML='<label class="tok-field">Acts as<select class="edActsAs"><option value="">— nobody (not a session token) —</option>'
     +PW_TOKEN_USERS.map(u=>'<option value="'+esc(u.username)+'"'+(u.username===t.actsAs?' selected':'')+'>'+esc(u.username)+'</option>').join('')+'</select></label>'
     +'<fieldset class="tok-picker"><legend>Scopes</legend>'+(PW_TOKEN_SCOPES||[]).map(sc=>'<label><input type="checkbox" class="edScope" value="'+esc(sc)+'"'+((t.scopes||[]).includes(sc)?' checked':'')+'>'+esc(sc)+'</label>').join('')+'</fieldset>'
     +'<fieldset class="tok-picker"><legend>Projects</legend><label class="tok-all"><input type="checkbox" class="edAll"'+(all?' checked':'')+'> All projects that account can reach</label><div class="tok-checks edProjects'+(all?' disabled':'')+'">'+(own.length?own.map(p=>'<label><input type="checkbox" class="edProject" value="'+esc(p)+'"'+(!all&&(t.projects||[]).includes(p)?' checked':'')+'>'+esc(p)+'</label>').join(''):'<span class="muted">'+esc(t.actsAs||'nobody')+' has no project grants.</span>')+'</div></fieldset>'
     +'<p class="muted">The secret is not changed, so anything already using this token keeps working with the new scopes.</p>'
     +'<button class="button tiny edSave" type="button">Save changes</button><span class="edStatus muted"></span>';
    const list=box.querySelector('.edProjects'),allBox=box.querySelector('.edAll'),who=box.querySelector('.edActsAs'),st=box.querySelector('.edStatus');
    const sync=()=>list.classList.toggle('disabled',allBox.checked||!who.value);
    allBox.addEventListener('change',sync);
    who.addEventListener('change',()=>{
     const mine=projectsFor(who.value);
     list.innerHTML=mine.length?mine.map(p=>'<label><input type="checkbox" class="edProject" value="'+esc(p)+'">'+esc(p)+'</label>').join(''):'<span class="muted">'+esc(who.value||'nobody')+' has no project grants.</span>';
     sync();
    });
    box.querySelector('.edSave').onclick=async()=>{
     const scopes=[...box.querySelectorAll('.edScope')].filter(c=>c.checked).map(c=>c.value);
     const projects=allBox.checked?'*':[...box.querySelectorAll('.edProject')].filter(c=>c.checked).map(c=>c.value);
     st.textContent='Saving…';
     try{
      const r=await fetch('${BASE}/api/tokens/'+encodeURIComponent(t.id),{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({scopes,actsAs:who.value,projects})});
      const j=await r.json();if(!j.ok)throw new Error(j.error||'update failed');
      st.textContent='';setStatus(tokStatus,'Updated "'+t.label+'" — the token itself is unchanged.');loadTokens();
     }catch(e){st.textContent=e.message}
    };
   }});
   tokList.querySelectorAll('.tokRevoke').forEach(b=>{b.onclick=async()=>{if(!confirm('Revoke this token? Any client using it stops working immediately.'))return;b.disabled=true;try{const rr=await fetch('${BASE}/api/tokens/'+encodeURIComponent(b.dataset.id)+'/revoke',{method:'POST'});const jj=await rr.json();if(!jj.ok)throw new Error(jj.error||'revoke failed');setStatus(tokStatus,'Token revoked.');loadTokens()}catch(e){setStatus(tokStatus,e.message,true);b.disabled=false}}})}catch(e){tokList.innerHTML='';setStatus(tokStatus,e.message,true)}}tokCreate.onclick=async()=>{const scopes=[...tokScopes.querySelectorAll('.tokScope')].filter(c=>c.checked).map(c=>c.value);const projects=tokAllProjects.checked?'*':[...tokProjectList.querySelectorAll('.tokProject')].filter(c=>c.checked).map(c=>c.value);
   if(projects!=='*'&&!projects.length){setStatus(tokStatus,'Choose at least one project, or tick "All projects".',true);return}tokCreate.disabled=true;setStatus(tokStatus,'Creating...');tokNew.hidden=true;try{const r=await fetch('${BASE}/api/tokens',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label:tokLabel.value,scopes,actsAs:tokActsAs.value.trim(),projects})});const j=await r.json();if(!j.ok)throw new Error(j.error||'create failed');tokNewVal.textContent=j.token;tokNew.hidden=false;showConnectInstructions(j.token,j.record);tokLabel.value='';tokActsAs.value='';tokAllProjects.checked=true;renderProjectPicker();tokScopes.querySelectorAll('.tokScope').forEach(c=>{c.checked=false});setStatus(tokStatus,'Token created. Copy it now.');loadTokens()}catch(e){setStatus(tokStatus,e.message,true)}finally{tokCreate.disabled=false}};tokCopy.onclick=async()=>{try{await navigator.clipboard.writeText(tokNewVal.textContent);setStatus(tokStatus,'Copied to clipboard.')}catch(e){setStatus(tokStatus,'Copy failed; select it manually.',true)}};
/* The instructions live HERE because this is the one moment they are needed and
   the one moment the token exists: the snippet is built with the real URL and the
   real secret, so nobody has to assemble it from prose. A token with no session
   scopes gets no MCP block — it cannot drive a session, and showing a config that
   would be refused would be worse than showing none. */
function showConnectInstructions(plaintext,record){
 const sessionScoped=(record&&record.scopes||[]).some(sc=>sc.indexOf('sessions:')===0);
 tokConnect.hidden=!sessionScoped;
 if(!sessionScoped)return;
 const origin=location.origin+'${BASE}';
 document.getElementById('tokActsAsEcho').textContent=record.actsAs||'that account';
 const config={mcpServers:{'project-workbench':{type:'http',url:origin+'/api/mcp',headers:{Authorization:'Bearer '+plaintext}}}};
 tokMcp.textContent=JSON.stringify(config,null,2);
 const project=(record.projects==='*'?'<project>':(record.projects||[])[0])||'<project>';
 const auth='-H "Authorization: Bearer '+plaintext+'"';
 const api=origin+'/api/agent/'+project+'/sessions/bot-lane';
 const quote="'";
 const body='-d '+quote+'{"prompt":"summarise the last deploy failure","cli":"claude"}'+quote;
 tokCurl.textContent=[
  '# 1. what this token can reach',
  'curl -fsS '+auth+' '+origin+'/api/agent/projects',
  '',
  '# 2. send a prompt (the session is created if missing, and cli is required then)',
  'TURN=$(curl -fsS '+auth+' -H "Content-Type: application/json" '+body+' '+api+'/prompt | jq -r .turn_id)',
  '',
  '# 3. wait for that turn to end, then read what it produced',
  'curl -fsS '+auth+' "'+api+'/turns/$TURN?wait_ms=120000"',
  'curl -fsS '+auth+' "'+api+'/output?since_turn=$TURN"'
 ].join(String.fromCharCode(10));
}
const copyFrom=(btn,el,what)=>{btn.onclick=async()=>{try{await navigator.clipboard.writeText(el.textContent);setStatus(tokStatus,what+' copied to clipboard.')}catch(e){setStatus(tokStatus,'Copy failed; select it manually.',true)}}};
copyFrom(document.getElementById('tokCopyMcp'),tokMcp,'MCP config');
copyFrom(document.getElementById('tokCopyCurl'),tokCurl,'curl example');loadTokens();
/* ---- scheduled tasks ---- */
...
</div></section><section id="tab-tokens"><h2>API tokens</h2><p class="lead">Bearer credentials for the machine API, so a script or an agent on another machine can register a project without an interactive sign-in. A token carries only the scopes you grant it &mdash; it is <b>not</b> an admin session and cannot reach user management, permission modes, or CLI installs.</p><div class="s-card"><h3>Create a token</h3><div class="tok-form"><label class="tok-field" for="tokLabel">Label<input id="tokLabel" placeholder="e.g. laptop-copilot" maxlength="80"></label><label class="tok-field" for="tokActsAs">Acts as<select id="tokActsAs"><option value="">&mdash; nobody (not a session token) &mdash;</option></select><span class="tok-hint">Required for any <code>sessions:*</code> scope. A session runs as somebody: this decides whose Claude/Copilot credentials a created window spends and whose name the audit line carries. Authority is the intersection of the token and that account &mdash; disable the account and every token acting as it stops in the same instant.</span></label><fieldset id="tokProjects" class="tok-picker"><legend>Projects</legend><label class="tok-all"><input type="checkbox" id="tokAllProjects" checked> All projects that account can reach</label><div id="tokProjectList" class="tok-checks"></div></fieldset><fieldset id="tokScopes" class="tok-picker"><legend>Scopes</legend></fieldset></div><button class="button" id="tokCreate" type="button">Create token</button><div id="tokNew" class="tok-new" hidden><p><b>Copy this now &mdash; it is shown once and cannot be retrieved again.</b></p><pre id="tokNewVal"></pre><button class="button secondary tiny" id="tokCopy" type="button">Copy the token</button><div id="tokConnect" hidden><h4>Connect an AI to this workbench</h4><p class="muted">Add this to the assistant's MCP configuration &mdash; Claude Code: <code>~/.claude.json</code> or <code>claude mcp add</code>; anything else: wherever it keeps <code>mcpServers</code>. It gives that assistant six tools: list projects and sessions, send a prompt to a named session, wait for the turn to end, and read what it produced.</p><pre id="tokMcp"></pre><button class="button secondary tiny" id="tokCopyMcp" type="button">Copy the MCP config</button><p class="muted tok-warn">That snippet contains the token. Treat the config file as a secret: anyone holding it can drive sessions as <span id="tokActsAsEcho">that account</span>, and this is the only time the token is displayed.</p><p class="muted">No MCP client? The same operations are plain HTTPS:</p><pre id="tokCurl"></pre><button class="button secondary tiny" id="tokCopyCurl" type="button">Copy the curl example</button></div></div><div id="tokStatus" class="muted"></div></div><div class="s-card"><h3>Existing tokens</h3><p class="muted">Revoking disables a token immediately; the record is kept so the audit trail outlives it. Sensitive events are logged to <code>/var/log/project-workbench/audit.log</code>.</p><div id="tokList" class="muted">loading&hellip;</div></div></section><section id="tab-system"><h2>System &amp; Updates</h2><p class="lead">Self-repair, version info, and a readiness checklist.</p><div class="s-card"><h3>Versions</h3><div id="sysVer" class="muted">loading…</div></div><div class="s-card"><h3>Readiness checklist</h3><ul class="check-list" id="sysChecks"><li class="muted">loading…</li></ul></div><div class="s-card"><h3>Heal</h3><p class="muted">Regenerate the nginx config from <code>projects.json</code>, or re-create runtime dirs / wrapper symlink if something looks broken.</p><button class="button" id="healNginxBtn" type="button">Regenerate nginx + reload</button> <button class="button secondary" id="healDirsBtn" type="button">Verify runtime dirs / wrapper</button>${PER_USER_CLAUDE ? ' <button class="button secondary" id="healLabelsBtn" type="button">Label existing terminals</button>' : ''}<pre class="heal-out" id="healOut"></pre>${PER_USER_CLAUDE ? '<p class="muted" style="margin-top:.6rem">A terminal opened before per-person identity existed runs on good credentials but never recorded whose, so its tabs show uncoloured. Labelling writes that record in place &mdash; it is a tmux option, so nothing running is disturbed. Only sessions whose stamped credentials still match their owner are labelled; anything that cannot be proven is skipped and named.</p>' : ''}</div><div class="s-card"><h3>Audit log</h3><p class="muted">Sensitive events are appended as JSONL to <code>/var/log/project-workbench/audit.log</code>. Tail it from a shell: <code>sudo tail -F /var/log/project-workbench/audit.log</code></p></div></section>
...
// ---- Machine API tokens (Settings -> API tokens) ---------------------------
// Administered from the dashboard by an admin session; USED only as a bearer credential on the
// machine surface. Creation returns the plaintext exactly once - only a SHA-256 digest is stored,
// so this list can never hand back a usable credential.
// Minting and editing a token are the same authority decision, so they share one
// check. A second copy would drift, and the thing that drifted would be whether a
// credential may act as somebody.
//
// The acting account is verified against the live user list rather than trusted
// from the form: a token naming a user who does not exist fails on first use with
// nothing to point at, and one naming a disabled account is a credential for
// somebody who has been offboarded. Project names are verified for the same
// reason — a typo is a token that silently never matches anything.
async function validateTokenGrant({ scopes = [], actsAs, projects }){
 const acting = String(actsAs || '').trim();
 if(needsActingUser(scopes)){
  if(!acting) return { error:'A token with session scopes must name the user it acts as' };
  const user = (await loadUsers()).find(u => u.username === acting);
  if(!user) return { error:`No such user: ${acting}` };
  if(user.disabled) return { error:`${acting} is disabled; a token cannot act as a disabled account` };
 }
 const scoped = projects === '*' || projects === undefined ? '*'
  : (Array.isArray(projects) ? projects.map(String).map(x => x.trim()).filter(Boolean) : []);
 if(scoped !== '*'){
  if(!scoped.length) return { error:'Choose at least one project, or "*" for everything that account can reach' };
  const known = new Set((await loadProjects()).map(p => p.name));
  const missing = scoped.filter(name => !known.has(name));
  if(missing.length) return { error:'Unknown project(s): '+missing.join(', ') };
  // Listing a project the acting account cannot reach is not a wider token — the
  // runtime check intersects the two — it is a tick that silently means nothing.
  // Refusing it here is the difference between a grant and a misunderstanding.
  if(acting){
   const user = (await loadUsers()).find(u => u.username === acting);
   const unreachable = user ? scoped.filter(name => !userHasProjectAccess(user, name)) : scoped;
   if(unreachable.length){
    return { error:`${acting} cannot reach: ${unreachable.join(', ')} — a token cannot exceed the account it acts as` };
   }
  }
 }
 return { actsAs: acting, projects: scoped };
}

app.get(BASE + '/api/tokens', requireAdmin, async (_req,res) => {
 try {
  const tokens = await loadTokens(apiTokensPath);
  // The pickers are populated from here rather than from a second round trip: the
  // acting account is a choice among real users, and a project list typed by hand
  // is a token that silently never matches. Names only — this route is admin-only
  // but a user list is not a user record.
  const [users, projects] = await Promise.all([loadUsers(), loadProjects()]);
  const names = projects.map(p => p.name).sort();
  // Per user, not a flat list: a token can never exceed the account it acts as, so
  // offering a project that account cannot reach would offer a tick that means
  // nothing. The picker is the intersection rule made visible.
  res.json({ ok:true, scopes:Object.values(SCOPES), tokens: tokens.map(safeTokenShape),
   users: users.filter(u => !u.disabled).map(u => ({
    username: u.username,
    projects: names.filter(name => userHasProjectAccess(u, name)),
   })).sort((a,b) => a.username.localeCompare(b.username)) });
 } catch(e){ res.status(500).json({ ok:false, error:e.message||String(e) }); }
});

app.post(BASE + '/api/tokens', requireAdmin, async (req,res) => {
 try {
  const label = String(req.body?.label || '').trim();
  const scopes = Array.isArray(req.body?.scopes) ? req.body.scopes.map(String) : [];
  if(!label) return res.status(400).json({ ok:false, error:'A label is required so the token is identifiable later' });
  const unknown = scopes.filter(x => !isKnownScope(x));
  if(unknown.length) return res.status(400).json({ ok:false, error:'Unknown scope(s): '+unknown.join(', ') });
  if(!scopes.length) return res.status(400).json({ ok:false, error:'Select at least one scope - a token with none can do nothing' });

  const grant = await validateTokenGrant({ scopes, actsAs: req.body?.actsAs, projects: req.body?.projects });
  if(grant.error) return res.status(400).json({ ok:false, error:grant.error });

  const { plaintext, record } = mintToken({ label, scopes, createdBy: req.user?.username || '',
   actsAs: grant.actsAs, projects: grant.projects });
  await withApiTokensLock((tokens) => { tokens.push(record); return true; });
  await audit('api_token_create', { tokenId:record.id, label, scopes, actsAs: record.actsAs || undefined,
   projects: record.projects === '*' ? '*' : record.projects }, req);
  // The ONLY time the plaintext is ever returned.
  res.json({ ok:true, token:plaintext, record:safeTokenShape(record) });
 } catch(e){ res.status(400).json({ ok:false, error:e.message||String(e) }); }
});

// Revoke = disable, never delete: the audit trail must outlive the credential.
// Change what a token may do WITHOUT rolling the secret.
//
// The digest is never touched, so a client already holding the credential keeps
// working — which is the whole point: widening or narrowing a bot's authority
// should not mean redeploying it. It is also why the audit line records the scopes
// on BOTH sides: adding a scope silently widens a credential that is already out
// there, and "what changed" is the question somebody will ask later.
app.patch(BASE + '/api/tokens/:id', requireAdmin, async (req,res) => {
 try {
  const id = String(req.params.id || '');
  const existing = (await loadTokens(apiTokensPath)).find(t => t.id === id);
  if(!existing) return res.status(404).json({ ok:false, error:'No such token' });

  const scopes = Array.isArray(req.body?.scopes) ? req.body.scopes.map(String) : existing.scopes || [];
  const unknown = scopes.filter(x => !isKnownScope(x));
  if(unknown.length) return res.status(400).json({ ok:false, error:'Unknown scope(s): '+unknown.join(', ') });
  if(!scopes.length) return res.status(400).json({ ok:false, error:'A token with no scopes can do nothing; revoke it instead' });

  // Adding a session scope to a token that names nobody has to name somebody in
  // the same edit — the same rule minting follows, checked by the same code.
  const grant = await validateTokenGrant({
   scopes,
   actsAs: req.body?.actsAs === undefined ? existing.actsAs : req.body.actsAs,
   projects: req.body?.projects === undefined ? existing.projects : req.body.projects,
  });
  if(grant.error) return res.status(400).json({ ok:false, error:grant.error });

  const label = req.body?.label === undefined ? existing.label : String(req.body.label).trim().slice(0,80);
  if(!label) return res.status(400).json({ ok:false, error:'A label is required so the token is identifiable later' });

  let updated = null;
  await withApiTokensLock((tokens) => {
   const record = tokens.find(t => t.id === id);
   if(!record) return false;
   // Everything else is immutable on purpose: the digest, so the secret survives;
   // id/createdAt/createdBy, because provenance is not editable history.
   record.label = label;
   record.scopes = [...new Set(scopes)];
   record.actsAs = grant.actsAs;
   record.projects = grant.projects;
   record.updatedAt = new Date().toISOString();
   record.updatedBy = req.user?.username || '';
   updated = safeTokenShape(record);
   return true;
  });
  if(!updated) return res.status(404).json({ ok:false, error:'No such token' });

  await audit('api_token_update', { tokenId:id, label,
   scopesBefore: existing.scopes || [], scopesAfter: updated.scopes,
   actsAsBefore: existing.actsAs || '', actsAsAfter: updated.actsAs,
   projectsBefore: existing.projects === '*' ? '*' : (existing.projects || []),
   projectsAfter: updated.projects }, req);
  res.json({ ok:true, record: updated });
 } catch(e){ res.status(400).json({ ok:false, error:e.message||String(e) }); }
});

app.post(BASE + '/api/tokens/:id/revoke', requireAdmin, async (req,res) => {
 try {
  const id = String(req.params.id||'');
  let found = null;
  await withApiTokensLock((tokens) => {
   ...
   if(!t) return false;
   t.disabled = true; t.revokedAt = new Date().toISOString(); t.revokedBy = req.user?.username || '';
   found = t; return true;
  });
  if(!found) return res.status(404).json({ ok:false, error:'No such token' });
  await audit('api_token_revoke', { tokenId:id, label:found.label }, req);
  res.json({ ok:true });
 } catch(e){ res.status(500).json({ ok:false, error:e.message||String(e) }); }
});
...
app.get(BASE + '/api/system/status', requireAdmin, async (_req,res) => {
 try {
  ...
  const checks = {
   ...
   // gh has now gone missing more than once WITHOUT anything noticing: the Containerfile
   // installs it, but the running image predated that layer, and every runtime install
   // since was thrown away with the container's writable layer on the next recreate. An
   // agent then discovered it mid-task. A missing tool that nothing reports is the part
   // worth fixing — deploy/install-gh.sh puts it on the persistent mount, and this line
   // is what says so if it ever evaporates again.
   ghInstalled: !!(await getCliVersion('gh')),
  };
