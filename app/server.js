...
app.get('/', async (_req,res)=>{
 ...
 res.type('html').send(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Project Workbench</title><style>${homeCss}${wizardCss}</style></head><body><header class="hero"><div><h1>Project Workbench</h1><p class="subtitle">Protected project terminals with Claude Code CLI</p></div><div class="hero-actions"><div class="meta-row"><span class="badge">Claude Code: <b>${esc(claudeVersion)}</b></span></div><div class="subtle">Last update check: ${esc(updateStamp)}</div><div class="action-row"><button id="editOrderBtn" class="button secondary pencilBtn iconBtn" type="button" title="Drag cards to reorder" aria-label="Edit order">✎</button><button id="setupBtn" class="button secondary" type="button">Setup Wizard</button><a class="button secondary" href="/manage">Manage Projects</a></div></div></header><div class="grid order-grid">${rows || '<p>No projects configured.</p>'}</div>${wizardModalHtml}<script>async function pwRefreshStatus(){try{const r=await fetch('/api/projects/status',{cache:'no-store'});const out=await r.json();if(!out?.ok)return;const map=Object.create(null);for(const p of out.projects)map[p.name]=p;document.querySelectorAll('article[data-project]').forEach(a=>{const s=map[a.dataset.project];a.classList.toggle('pending',!!(s&&s.pending))})}catch{}}pwRefreshStatus();setInterval(()=>{if(!document.hidden)pwRefreshStatus()},5000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)pwRefreshStatus()});</script>${reorderScript}${wizardScript}</body></html>`);
});
