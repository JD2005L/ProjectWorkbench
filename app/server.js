...
// PW force-motion standard: show animations regardless of the OS "reduce motion"
// setting (spuriously on for RDP/VMs/"best performance" Windows). Patches matchMedia
// for JS motion libs + strips "@media (prefers-reduced-motion: reduce)" CSS kill-switches.
// Canonical copy + rationale: standards/force-motion/. Injected first-in-<head> on every page.
const FORCE_MOTION_HEAD = `<script>/*pw-force-motion*/(function(){"use strict";try{var n=window.matchMedia;if(n){n=n.bind(window);var f=function(q){return{media:q,matches:/no-preference/i.test(q),onchange:null,addListener:function(){},removeListener:function(){},addEventListener:function(){},removeEventListener:function(){},dispatchEvent:function(){return false}}};window.matchMedia=function(q){return(typeof q==="string"&&/prefers-reduced-motion/i.test(q))?f(q):n(q)}}}catch(e){}function w(s){var r;try{r=s.cssRules}catch(e){return}if(!r)return;for(var i=r.length-1;i>=0;i--){var x=r[i],c=(x&&(x.conditionText||(x.media&&x.media.mediaText)))||"";if(x&&x.type===4&&/prefers-reduced-motion\\s*:\\s*reduce/i.test(c)){try{s.deleteRule(i)}catch(e){}}else if(x&&x.styleSheet){w(x.styleSheet)}}}function run(){var s=document.styleSheets;for(var i=0;i<s.length;i++)w(s[i])}run();document.addEventListener("DOMContentLoaded",run);window.addEventListener("load",run)})();</script>`;
...
app.get('/settings', requireAdmin, async (req,res) => {
 ...
 const updateStamp = await getClaudeUpdateStamp();
 const footer = statusBarHtml({ claudeVersion, updateStamp, user: req.user, enforce: AUTH_ENFORCE });
 res.type('html').send(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">${FORCE_MOTION_HEAD}<title>Settings — Project Workbench</title><style>${settingsCss}${statusBarCss}${modalBaseCss}${wizardCss}</style></head><body><header class="s-header"><a class="back" href="/">← Dashboard</a><h1>Settings</h1><span class="grow"></span><span class="who"><b>${esc(req.user.username)}</b> · ${esc(req.user.role)}</span></header><div class="s-layout"><nav class="s-tabs"><button data-tab="users" class="active">Users &amp; Roles</button><button data-tab="clis">CLIs &amp; Sign-in</button><button data-tab="env">Environment</button><button data-tab="system">System &amp; Updates</button><button data-tab="firstrun">First Run</button></nav><main class="s-main">
