...
async function ensureSetupTerminal(){
 const initial = await diagnoseSetupTerminalService();
 if(!initial.ok && initial.props?.LoadState === 'not-found') return { ok:false, error: initial.hint };
 ...
 for(let i=0;i<20;i++){
  try { await tmux(['has-session','-t',setupTmuxSession]); return { ok:true }; } catch {}
  await new Promise(r=>setTimeout(r,150));
 }
 const after = await diagnoseSetupTerminalService();
 if(!after.ok) return { ok:false, error: after.hint };
 const logs = await serviceLogs('project-setup-terminal.service',20);
 return { ok:false, error:[`project-setup-terminal.service started but tmux session ${setupTmuxSession} never appeared.`,'Inspect:','sudo systemctl status project-setup-terminal.service --no-pager','sudo journalctl -u project-setup-terminal.service -n 50 --no-pager',logs ? `Recent logs:\n${logs}` : ''].filter(Boolean).join('\n\n') };
}
