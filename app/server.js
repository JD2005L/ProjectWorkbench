...
.leftInfo{display:flex;align-items:center;gap:10px;min-width:0;flex:0 1 auto}
.brand{display:inline-flex;align-items:center;gap:7px;color:var(--dim);text-decoration:none;font-weight:700;font-size:11.5px;letter-spacing:.14em;text-transform:uppercase;padding:4px 8px;border-radius:8px;border:1px solid transparent;white-space:nowrap}
.brand:hover{color:#fff;border-color:var(--line);background:rgba(255,255,255,.03)}
.brandGlyph{display:inline-grid;place-items:center;width:22px;height:22px;border-radius:6px;background:linear-gradient(135deg,var(--cyan),var(--blue));color:#04101f;font:900 9px/1 var(--mono);letter-spacing:0}
.projChip{display:inline-flex;align-items:center;gap:8px;min-width:0;padding:3px 11px 3px 4px;border:1px solid var(--line);border-radius:999px;background:#0d1526}
.pcMono{display:inline-grid;place-items:center;width:24px;height:24px;border-radius:999px;font:700 10.5px/1 var(--mono);background:hsl(var(--h) 55% 16%);color:hsl(var(--h) 90% 72%);border:1px solid hsl(var(--h) 55% 30%)}
.pcName{font-weight:650;font-size:13px;color:#f4f8ff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:240px}
...
#rail{order:2;flex:0 0 var(--rail-w);width:var(--rail-w);display:flex;flex-direction:column;background:linear-gradient(180deg,#0a1120,#070c16 55%,#0a1120);border-left:1px solid var(--line);transition:width .32s cubic-bezier(.32,.72,.24,1),flex-basis .32s cubic-bezier(.32,.72,.24,1);overflow:hidden;position:relative;z-index:14}
body.rail-open #rail{flex-basis:var(--rail-wo);width:var(--rail-wo)}
#rail::before{content:'';position:absolute;inset:0;pointer-events:none;background:radial-gradient(120% 40% at 100% 0%,rgba(56,189,248,.05),transparent 60%)}
.railHead{display:flex;align-items:center;gap:8px;height:var(--topbar-h);flex:0 0 var(--topbar-h);padding:0 12px;border-bottom:1px solid var(--line);box-sizing:border-box}
#railToggle{width:30px;height:30px;flex:0 0 30px;display:grid;place-items:center;background:#0d1526;border:1px solid var(--line);border-radius:8px;color:var(--dim);cursor:pointer;font-size:15px;transition:color .2s,border-color .2s,background .2s;padding:0;margin-left:-3px}
#railToggle:hover{color:#fff;border-color:var(--line2);background:var(--panel2)}
#railToggle .chev{display:inline-block;transition:transform .32s cubic-bezier(.32,.72,.24,1)}
body.rail-open #railToggle .chev{transform:rotate(180deg)}
.railTitle{font-size:11px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--faint);opacity:0;transform:translateX(8px);transition:opacity .25s,transform .25s;white-space:nowrap}
body.rail-open .railTitle{opacity:1;transform:none}
.railKeys{flex:1 1 auto;overflow-y:auto;overflow-x:hidden;padding:10px 0 10px 8px;display:flex;flex-direction:column;gap:7px;scrollbar-width:thin;scrollbar-color:var(--line) transparent}
.pkey{position:relative;display:flex;align-items:center;gap:9px;height:46px;flex:0 0 46px;padding:0 8px 0 10px;border:1px solid var(--line);border-right:0;border-radius:11px 0 0 11px;background:linear-gradient(180deg,var(--panel),#0a1120);color:var(--dim);text-decoration:none;box-sizing:border-box;transition:transform .18s cubic-bezier(.34,1.4,.44,1),background .18s,border-color .18s,box-shadow .18s,color .18s;animation:pkIn .5s cubic-bezier(.22,.9,.3,1) both;animation-delay:calc(var(--i)*38ms)}
...
.pk-edge{position:absolute;left:0;top:8px;bottom:8px;width:3px;border-radius:3px;background:hsl(var(--h) 50% 34% / .85);transition:background .2s,box-shadow .2s}
...
body.rail-open .pk-meta{opacity:1;transform:none}
...
.pk-fly{position:absolute;right:calc(100% + 10px);top:50%;transform:translateY(-50%) translateX(6px);background:#0d1526;border:1px solid var(--line2);color:#eaf2ff;font-size:12px;font-weight:600;padding:5px 10px;border-radius:8px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .18s,transform .18s;box-shadow:0 10px 26px rgba(0,0,0,.7);z-index:30}
body:not(.rail-open) .pkey:hover .pk-fly{opacity:1;transform:translateY(-50%)}
...
body.rail-open .railActLabel{opacity:1;transform:none}
.railWho{display:flex;align-items:center;gap:2px;padding:4px 4px 2px;color:var(--faint);font-size:11.5px;min-width:0;align-items:center}
...
body.rail-open .railWhoName{opacity:1}
#railScrim{display:none}
@media(max-width:640px){
#rail{position:fixed;top:0;right:0;bottom:0;z-index:60;transform:translateX(100%);transition:transform .3s cubic-bezier(.32,.72,.24,1);width:var(--rail-wo);flex-basis:var(--rail-wo);border-left:1px solid var(--line2);box-shadow:-24px 0 60px rgba(0,0,0,.75)}
...
function railHtml(projects, currentName, user){
 ...
 const keys = projects.map((p,i)=>{
  ...
  return `<a class="pkey${cur?' current':''}" href="/term/${encodeURIComponent(p.name)}/" data-project="${esc(p.name)}" style="--h:${projHue(p.name)};--i:${i}"${cur?' aria-current="page"':''}><span class="pk-edge"></span><span class="pk-mono">${esc(projMonogram(p.name))}</span><span class="pk-meta"><span class="pk-name">${esc(p.name)}</span><span class="pk-sub">${cur?'active session':''}</span></span><span class="pk-dot" aria-hidden="true"></span><span class="pk-fly" aria-hidden="true">${esc(p.name)}</span></a>`;
 }).join('');
 ...
 return `<aside id="rail" aria-label="Projects"><div class="railHead"><button id="railToggle" type="button" aria-expanded="false" title="Expand projects"><span class="chev" aria-hidden="true">‹</span></button><span class="railTitle">Projects</span></div><nav id="railKeys" class="railKeys">${keys}</nav><div class="railFoot">${adminActs}${who}</div></aside><div id="railScrim" aria-hidden="true"></div>`;
}
...
function setOpen(open,persist){document.body.classList.toggle('rail-open',open);toggle.setAttribute('aria-expanded',open?'true':'false');toggle.title=open?'Collapse projects':'Expand projects';if(persist){try{localStorage.setItem('pwRailOpen',open?'1':'0')}catch{}}}
...
const CUR=(typeof project!=='undefined')?project:null;
let cfg=null,sel=null,mode='edit',formDirty=false,reloadOnClose=false,navTarget=null,busy=false;
...
if(mode==='add'){const name=fName.value.trim();setStatus('Creating'+(fRepo.value.trim()?' — cloning can take a minute…':'…'));const params=new URLSearchParams({name,repo:fRepo.value.trim(),port:fPort.value||''});await api('/manage/add',params);reloadOnClose=true;setStatus('Created '+name+'.');await refreshCfg();sel=name;mode='edit';select(name)}
else{const oldName=sel;const newName=fName.value.trim();setStatus('Saving — restarting terminal service…');const params=new URLSearchParams({name:newName,repo:fRepo.value.trim(),port:fPort.value||'',previewCmd:fPrevCmd.value,previewPort:fPrevPort.value||'',previewEnv:fPrevEnv.value,tabs:JSON.stringify(collectTabs())});await api('/manage/update/'+encodeURIComponent(oldName),params);reloadOnClose=true;if(oldName===CUR&&newName!==CUR)navTarget='/term/'+encodeURIComponent(newName)+'/';await refreshCfg();sel=newName;formDirty=false;select(newName);setStatus('Saved '+newName+' — terminal restarted.')}
}catch(e){setStatus(e.message||String(e),true)}finally{busy=false;saveBtn.disabled=false}};
delBtn.onclick=async()=>{if(busy||delBtn.disabled)return;if(!confirm('Really delete "'+sel+'" AND its workspace folder? This cannot be undone.'))return;busy=true;delBtn.disabled=true;try{setStatus('Deleting '+sel+'…');await api('/manage/delete/'+encodeURIComponent(sel),new URLSearchParams({confirm:'yes'}));reloadOnClose=true;if(sel===CUR)navTarget='/';await refreshCfg();setStatus('Deleted.');sel=null;formDirty=false;if(cfg.projects.length)select(cfg.projects[0].name);else{navTarget=navTarget||'/';closeModal()}}catch(e){setStatus(e.message||String(e),true)}finally{busy=false}};
let dragSrc=null;
...
items.addEventListener('dragend',async()=>{if(!dragSrc)return;dragSrc.classList.remove('dragging');dragSrc=null;const order=[...items.querySelectorAll('.pmItem')].map(r=>r.dataset.name);try{await fetch('/api/projects/reorder',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({order})});reloadOnClose=true;cfg.projects.sort((a,b)=>order.indexOf(a.name)-order.indexOf(b.name));setStatus('Order saved — the rail follows it.')}catch(e){setStatus('Reorder failed: '+(e.message||e),true)}});
...
app.get('/', requireAuth, async (req,res)=>{
 ...
 if(isAdmin){
  const [cv, users, cAuth] = await Promise.all([getClaudeVersion(), loadUsers(), getCliAuth('claude')]);
  firstRunNeeded = cv === 'unavailable' || !cAuth || users.length === 0;
 }
 ...
 // switching. `/` only renders as a page for first-run onboarding, an empty
 // registry, or roles that cannot open terminals.
 if(canOpenTerminal && projects.length && !firstRunNeeded){
 ...
 let content;
 if(canOpenTerminal){
  const addStep = projects.length ? '' : `<div class="lStep"><span class="lNum">2</span><div class="lStepMeta"><b>Add your first project</b><span>Clone a repo — or start an empty local workspace — and land straight in its cockpit.</span></div><button class="lBtn primary" id="openManageBtn" type="button">Add project</button></div>`;
  content = `<section class="lCard lOnboard" style="--i:0"><h2>Let's get this workbench running</h2><p class="lLead">${projects.length ? 'Finish signing in a CLI and you will land straight in your cockpit.' : 'Two steps and you land in your first cockpit.'}</p><div class="lStep"><span class="lNum">1</span><div class="lStepMeta"><b>Sign in your AI CLI</b><span>Install and authenticate Claude Code (or Codex / Copilot); tokens apply to every project terminal.</span></div><button class="lBtn${projects.length ? ' primary' : ''}" id="openWizardBtn" type="button">Open Setup Wizard</button></div>${addStep}</section>`;
...
app.get('/files/:project/', requireInboxWrite, async (req,res)=>{
 ...
 const updateStamp = await getClaudeUpdateStamp();
 const footer = statusBarHtml({ claudeVersion, updateStamp, user: req.user, enforce: AUTH_ENFORCE });
 const projectJson = JSON.stringify(p.name);
