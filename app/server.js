...
import { normalizeTask, evaluateDue, resolveTargets, preserveBookkeeping, guardedGitCommand, SCHEDULE_LIMITS, NORTH_AMERICAN_TIMEZONES } from './scheduled-tasks.js';
...
app.get(BASE + '/api/tasks', requireAdmin, async (_req,res)=>{ try {
 ...
 const tz = await displayTimeZone();
 res.json({ ok:true, timeZone: tz, limits: SCHEDULE_LIMITS, timeZones: NORTH_AMERICAN_TIMEZONES,
...
async function loadTasks(){
 try{const r=await fetch('${BASE}/api/tasks',{cache:'no-store'});const j=await r.json();if(!j.ok)throw new Error(j.error||'load failed');
  taskState={tasks:j.tasks||[],projects:j.projects||[],gitTemplate:j.gitTemplate||'',timeZone:j.timeZone||'',timeZones:j.timeZones||[]};
  renderTzOptions();renderTasks();
