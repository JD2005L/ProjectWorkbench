...
### 2026-07-16 — detector v3 + pinned keys
- Working detection v3: live-sampled my own mid-turn session — streak model needed 2+ samples (8s+) of CONTINUOUS polling to arm, so unwatched projects rarely showed (user report). Insight: client-induced repaints only reach the ACTIVE window of an ATTACHED session; detached sessions and background windows cannot spontaneously repaint → stateless instant path for them, streak-hold (8s) only for active+attached. Detached tick loop now lights in 1.7s (was ~9s); idle-view false-positive test still 0/7. FRESH 3→5s.
...
### 2026-07-16 — detector v6: cadence model (final)
- User: working state dropped on refresh / not maintained through turns; then a viewed-idle project flagged working. v4 (hysteresis) regressed the idle-view guard: GRACE stretched any armed state; v5 (sustained advances ≥2 span ≥3s) still failed — debug tracing showed the compound noise sequence create(888)+echo(889/890)+attach-repaint(892) = 3 advances over 4s legitimately passing every threshold, then stay refreshed by any later stamp.
- v6 discriminates by CADENCE: stamp changes vs sample slots over a 12s sliding window. Enter: ≥3 changes spanning ≥5s w/ output ≤6s old; active+attached additionally ≥70% of ≥4 slots spanning ≥8s (where viewer repaints live). Stay: grace 15s from last sustained observation; ends at bell or when cadence stops.
- Verified: idle-view 0/7 (stamps frozen, workdbg trace), detached tick lights 5.1s, attached enters ~3-10s, holds through 7s pause, ON immediately after page refresh (state is server-side), off ~15-22s after stop; live check: only ProjectWorkbench (this genuinely-working session) flagged across samples. Sweep 29/29. Debug tap left in code behind PW_WORK_DEBUG env (drop-in removed).

### 2026-07-16 — done-state made server-authoritative (latch) + amber-over-pin
- User: auto-pinned projects didn't show orange; asked whether we're done whack-a-moling. Two real defects:
  1) CSS: .pkey.pinned border/glow overrode .pkey.lit → every auto-pinned done key lost its amber cue. Fix: .pkey.pinned.lit amber border+glow (lean kept). pintest asserts computed borderColor rgb(161,98,7) (with a 500ms transition-settle — first assert raced the 180ms transition).
  2) Done-state fragility: one-shot tmux bell flag clearable by ANY attach (stray tabs, tests, PVIKPBot automation selecting windows); pre-hook-update sessions never write markers. Fix: server LATCHES observed bells into the pending marker (root-owned; hook markers admin-owned — ownership identifies the writer). Latch gated: never while attached, and suppressed 12s after any clearPending (the cockpit GET clears ~500ms before the iframe attaches and the page's first poll lands in that gap — caught twice by the sweep before the suppression fix). Marker clears only on genuine view (GET + visible heartbeat).
- Sweep collisions during verification were REAL activity (PVIKPBot finished turns at 04:48:51 and intermittently — admin-owned markers = hook writes): tests sharing AmrikPublic with a live bot.
- Final: pw-verify 31/31 (new C3c-latch + C3c-clears-on-view), pintest incl. amber-precedence, worktest2 (idle-view 0/7), worktest3 (pause/refresh continuity; bell-less off ≤25s documented).
- State model now server-authoritative: working = cadence tracker (server), done = latched marker (server), current = server-rendered; client renders only. Pins = localStorage by design.
